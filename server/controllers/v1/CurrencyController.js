const CurrencyMaster = require("../../models/CurrencyMaster.js");
const { getReferencingCounts,
  formatReferenceMessage,
 } = require("../../utils/referenceHelper.js");

// Create CurrencyMaster
const createCurrencyMaster = async (req, res) => {
  try {
...
const getCurrencyMasterById = async (req, res) => {
  try {
    const { id } = req.params;
...
const updateCurrencyMaster = async (req, res) => {
  try {
    const { id } = req.params;
    const { currencyName, currencyCode, currencySymbol, isActive } = req.body;
...
const deleteCurrencyMaster = async (req, res) => {
  try {
    const { id } = req.params;
...
const listCurrencyMastersByParams = async (req, res) => {
  try {
    const {
      skip = 0,
      per_page = 100,
...
const getAllActiveCurrencyMasters = async (req, res) => {
  try {
