...
const Employee = () => {
      ...
      selector: (row) => {
        return (
          <div className="d-flex gap-2">
            {(currentPagePermissions.edit || role === "ADMIN") && <button
            ...
            {(currentPagePermissions.delete || role === "ADMIN") && <button
  ...
  const validate = (values) => {
    ...
    if (values.pincode && !/^\d{6}$/.test(values.pincode)) errors.pincode = "Pincode must be exactly 6 digits";
    ...
    else if (values.mobileNumber.length !== 10) errors.mobileNumber = "Mobile Number should be 10 digits";
  ...
  const handleList = () => {
    ...
    setIsSubmit(false);
    setValues(initialState);
    // setSelectedDepartment(null); // Removed legacy invalid call
    setSelectedRole(null);
    setFormErrors({});
    setShowResetPassword(false);
    // Reset country/state/city lists
              ...
              <Card>
                <CardHeader>
                  <FormsHeader
                    ...
                    showAddButton={currentPagePermissions.write || role === "ADMIN"}
                  />
                </CardHeader>
