    ...
    try {
      ...
      if (response.data.isOk) {
        ...
        if (Array.isArray(menuGroupsData)) {
          // If current user is NOT a super admin (developer), hide Menu Master and Menu Categories
          const isSuper = (adminData && adminData.isSuperAdmin) === true || localStorage.getItem("isSuperAdmin") === "true";

          let menuToSet = menuGroupsData;
          if (!isSuper) {
            menuToSet = menuGroupsData.map(group => {
              if (!group.menus) return group;
              const filtered = group.menus.filter(menu => {
                const url = (menu.url || "").toLowerCase();
                if (url === "/menu-master" || url === "/menu-group") return false;
